...
{
    public partial class TaxName : Form
    {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                cn.Open();
                SqlCommand command2 = new SqlCommand("SELECT ID,InternalName FROM Tax_Name where InternalName='" +txtName.Text + "'", cn);
                sqlDataReader = command2.ExecuteReader();
                if (!sqlDataReader.Read())
                {
                    cn.Close();
                    SqlCommand cmd2 = new SqlCommand("Insert Into Tax_Name (InternalName,TaxName,Description,ArabicDescription) OUTPUT INSERTED.ID Values (N'" + txtName.Text + "','" + cmbType.Text + "', '" + txtDescEn.Text + "',N'" + txtDescAr.Text + "')", cn);
                    Int32 id = 0;
                    cn.Open();
                    ...
                    for (int i = 0; i <= dgv.Rows.Count - 1; i++)
                    {
                        ...
                        {
                            cmd = new SqlCommand("Insert into Tax_SubTypes(SubType,Description,ArabicDescription,IDTaxName) Values (N'" + dgv.Rows[i].Cells[1].Value + "','" + dgv.Rows[i].Cells[2].Value + "',N'" + dgv.Rows[i].Cells[3].Value + "'," + id + ")", cn);
                ...
                {
                    cn.Close();
                    MessageBox.Show("هذه الضريبة موجوده بالفعل ");
                }
            }
        }
